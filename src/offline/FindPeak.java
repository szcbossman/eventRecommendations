...
//MapReduce method, 15min a bucket, one query a timeStamp, emit to corresponding bucket and later sum for peak.
...
public class FindPeak{
	...
	private static List<LocalTime> initBuckets() { //15min a atom level, prev a second, too small.
		List<LocalTime> buckets = new ArrayList<>();
		LocalTime time = LocalTime.parse("00:00");
		for (int i = 0; i < 96; ++i) { //24h
			buckets.add(time);
